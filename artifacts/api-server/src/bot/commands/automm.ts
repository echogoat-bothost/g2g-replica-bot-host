...
export async function handleAutommSetup(
  ...
  const traderIdInput = new TextInputBuilder()
    .setCustomId("trader_discord_id")
    .setLabel("Trader's Discord ID")
  ...
  modal.addComponents(
    new ActionRowBuilder<TextInputBuilder>().addComponents(traderIdInput),
...
export async function handleAutommTradeModal(
  ...
  const traderDiscordId = interaction.fields.getTextInputValue("trader_discord_id").trim();
  ...
  if (channel && traderDiscordId) {
    try {
      const guild = interaction.guild!;
      const member = await guild.members.fetch(traderDiscordId);
      ...
      await channel.send({
        content: `<@${member.id}> has been added to this ticket as the **trader**.`,
      });
    } catch {
      await channel.send({
        content: `⚠️ Could not find a member with Discord ID \`${traderDiscordId}\`. Please ask staff to add them manually with \`/add\`.`,
      });
    }
  }
  ...
  const walletEmbed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "Trader", value: `<@${traderDiscordId}>`, inline: true },
